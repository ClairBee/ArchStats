...
%\textbf{Removal of points not on a wall \nb{Maybe remove this if not useful in Catholme?}}

%A further cleaning step assumes that all of the features of interest will consist of straight lines of post-holes, either perpendicular or parallel to one another. For any post-hole $i$ that forms part of such a feature, then, we expect $i$ and its two nearest neighbours either to lie in a straight line or to form a right-angle at $i$. Thus, having obtained the angles $\phi_{i_1}$ and $\phi_{i_2}$ between $i$ and its two nearest neighbours $i_1$ and $i_2$, we can calculate the angular distance $d_2(i)$ between the two. In axial data, an angular difference of $0$ is equivalent to an angle of $\pi/2$ or $\pi$, so we will use
%\begin{equation}
%d_2(i) = \pi - \vert \pi - \vert \phi_{i_1} - \phi_{i_2} \vert \vert \text{ modulo }\pi/2
%\end{equation}
%The resulting angular distances can be used to filter the points, retaining only  $\{i:d_2(i) < \pi/8 \, \cup \, d_2(i) > 3\pi/8\}$. Any points whose nearest neighbours are separated by an angular distance (modulo $\pi/2$) closer to $\pi/4$ than to $0$ or $\pi/2$ are discarded; they and their two nearest neighbours do not form part of a linear or perpendicular structure, so it is unlikely that their orientation will provide any information about the orientation of the linear and perpendicular structures of the site.
