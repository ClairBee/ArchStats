...
\nb{Expand: The Jones-Pewsey's shape parameter $\psi$ allows the degree of concentration of the peak of the distribution to be more robust to the presence of angles at the antipode than the von Mises, which has to attempt to capture both factors in one concentration parameter.}
...
\todo{mixture model is not as useful for inference: bootstrap GoF tests are unreliable due to the difficulty of simulating from a mixture model, tending to rather extreme $p$ values. Reserve mixture models for }
